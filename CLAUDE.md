...
**IMPORTANT: Do not build the solution or any project (`dotnet build`, etc.) unless the user explicitly tells you to. This applies until the user says otherwise.**

## Git Commit Messages

**IMPORTANT: Never add a `Co-Authored-By` trailer (or any other co-author attribution) to commit messages in this repository.**
...
## WinUI 3 Reference Templates (SheduleHelper.Modern)

`SheduleHelper.Modern` targets `net10.0-windows10.0.26100.0` and uses WinUI 3 via `Microsoft.WindowsAppSDK` (not UWP `Windows.UI.Xaml`). Its actual runtime default styles/control templates (e.g. `DefaultProgressBarStyle`) come from that NuGet package, not from the OS-installed Windows SDK `DesignTime\...\Generic\generic.xaml` (that one is UWP-only and is never loaded by this app).

A local copy of the real, in-use generic.xaml is kept at `.claude/reference/WinUI3-generic.xaml` (gitignored, not committed) for quick lookup of default WinUI 3 control templates/resources without re-navigating the NuGet cache each time. It was copied from:

```
%userprofile%\.nuget\packages\microsoft.windowsappsdk\1.7.250606001\lib\net6.0-windows10.0.22621.0\Microsoft.WinUI\Themes\generic.xaml
```

This specific `net6.0-windows10.0.22621.0` folder is the one NuGet actually selects for this project (highest Windows-version asset not exceeding the project's target `10.0.26100.0`, among the `net6.0-windows*` folders — `uap10.0*` and `win10-*` folders in that package are irrelevant to this non-UWP, non-native-AOT app). If `Microsoft.WindowsAppSDK` is upgraded, re-copy this file from the new version's matching folder.
